...
path2mix = 'datasets/result_mix_1d.txt'
        ...
        while ss < len(line):
            ...
            if searchObj:
                ...
                if s > 0: # before searchObj
                    ...
                    print(f'after mm. mmlst={mmlst} remaining={remaining}')
                    if forward:
                        for mmword in mmlst:
                            outfile.write(mmword + '  ')
                        if remaining:
                            print(f'predicting {remaining} [{ss}:{s}]')
                            pred = seg.calc(remaining)
                            for i in range(s-len(remaining), s):
                                print(f'parsing i={i}')
                                outfile.write(line[i])
                                if pred[i-s+len(remaining)] == 'S' or pred[i-s+len(remaining)] == 'E':
                                    outfile.write('  ')
                    else: # backward
                        if remaining:
                            print(f'predicting {remaining} [{ss}:{s}]')
                            pred = seg.calc(remaining)
                            for i in range(ss, ss+len(remaining)):
                                print(f'parsing i={i}')
                                outfile.write(line[i])
                ...
                print(f'after mm. mmlst={mmlst} remaining={remaining} begin={ss} end={len(line)}')
                if forward:
                    for mmword in mmlst:
                        outfile.write(mmword + '  ')
                    if remaining:
                        print(f'predicting {remaining} [{ss}:{len(line)}]')
                        pred = seg.calc(remaining)
                        for i in range(len(line)-len(remaining), len(line)):
                            print(f'parsing i={i} ss={ss}')
                            outfile.write(line[i])
                            if pred[i-len(line)+len(remaining)] == 'S' or pred[i-len(line)+len(remaining)] == 'E':
                                outfile.write('  ')
                else: # backward
                    if remaining:
                        print(f'predicting {remaining} [{ss}:{len(line)}]')
